...
const riderRoute = factory
  ...
  .post("/orders/:orderId/accept", async (c) => {
    try {
      ...
      if (order.riderId) {
        return c.json({ error: "Order already assigned to another rider" }, 400);
      }
  ...
  .get("/orders/current", async (c) => {
    try {
      ...
      const currentOrder = await db.query.orderTable.findFirst({
        where: and(
          eq(orderTable.riderId, rider.id),
          inArray(orderTable.status, ["RIDER_ASSIGNED", "IN_TRANSIT"])
        ),
        with: {
          customer: true,
  ...
  .post("/orders/:id/deliver", zValidator("json", z.object({
    ...
    try {
      ...
      if (confirmationCode && order.riderConfirmationCode !== confirmationCode) {
