...
<main class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
	<div class="mb-8">
		...
		<h1 class="mt-4 text-3xl font-bold">Check out</h1>
		<p class="mt-2 text-gray-600">
			Authentic Mediterranean kebab and falafel restaurant with fresh ingredients and traditional
			recipes.
		</p>
					...
					<div class="space-y-4">
						<div class="flex items-center justify-between">
							<div class="flex items-center gap-2">
								<MapPin class="size-5" />
								<span>Other</span>
								<span class="text-sm text-muted-foreground">Estonia puiestee</span>
							</div>
	...
	>
		<div class="flex w-full flex-col gap-10 md:gap-10 lg:gap-20">
			...
			<div class="flex h-[auto] w-full flex-col gap-4 py-4 lg:max-w-[600px]">
				...
				<!-- <OrderList /> -->
				<div>
					<Button variant="secondary" href="/explore/stores">Add more items</Button>
				</div>
			</div>
					...
					<ul class="flex flex-col gap-3 border-b pb-5 pt-2">
						<li class="flex items-center justify-between">
							<p class="text-sm font-medium lg:text-base">
								Item subtotal ({mockOrder.lines.length} items)
							</p>
							<span class="rounded-md px-2 py-1 text-sm font-medium text-primary lg:text-base">
								{formatCurrency(mockOrder.subTotalWithTax, mockOrder.currencyCode)}
							</span>
						</li>
						<li class="flex items-center justify-between">
							<p class="text-sm font-medium lg:text-base">
