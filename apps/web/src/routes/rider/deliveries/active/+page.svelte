<script lang="ts">
	...
	let showConfirmationInput = $state(false);
	...
	async function markDelivered() {
		...
		isLoading = true;
		try {
			const success = await riderOrderService.markOrderDelivered(
				currentDelivery.id,
			...
			if (success) {
				...
				toast.error('Invalid confirmation code or delivery failed');
...
<div class="space-y-6">
	...
	{:else}
		<Card class="p-6">
			...
			<div class="grid gap-4 md:grid-cols-2">
				...
				<div class="space-y-3">
					...
					{#if currentDelivery.riderConfirmationCode && (currentDelivery.status === 'PICKED_UP' || currentDelivery.status === 'IN_TRANSIT')}
						<div
							class="rounded-lg border border-yellow-200 bg-yellow-50 p-3 dark:border-yellow-800 dark:bg-yellow-900/20"
						>
							<div class="mb-1 text-sm font-medium text-yellow-800 dark:text-yellow-200">
								Customer Confirmation Code:
							</div>
							<div
								class="font-mono text-2xl font-bold tracking-wider text-yellow-900 dark:text-yellow-100"
							>
								{currentDelivery.riderConfirmationCode}
							</div>
							<div class="mt-1 text-xs text-yellow-700 dark:text-yellow-300">
								Ask customer for this code to confirm delivery
							</div>
						</div>
					{/if}
					<div class="text-right">
						<div class="text-muted-foreground text-sm">Delivery Fee</div>
		...
		{:else if currentDelivery.status === 'PICKED_UP' || currentDelivery.status === 'IN_TRANSIT'}
			<div class="bg-background sticky bottom-4 space-y-3 rounded-lg border p-4 shadow-lg">
				{#if showConfirmationInput}
					<div class="space-y-2">
				...
				<Button class="w-full" size="lg" onclick={markDelivered} disabled={isLoading}>
					...
					{:else}
						<CheckCircle class="mr-2 h-4 w-4" />
					{/if}
					{showConfirmationInput ? 'Confirm Delivery' : 'Mark as Delivered'}
				</Button>
			</div>
		{/if}
	{/if}
