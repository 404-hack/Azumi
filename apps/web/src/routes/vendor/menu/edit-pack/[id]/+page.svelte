<!-- <script lang="ts">
	...
	import { menuPackSchema } from '@repo/types/src/menu';

	export let data;

	const form = superForm(data.pack, {
		validators: zod(menuPackSchema),
		SPA: true,
		onUpdate: async ({ form }) => {
			if (form.valid) {
				try {
					const res = await client.pack[':id'].$patch({
						param: { id: data.pack.id },
						json: {
							...form.data
						}
					});
					if (res.ok) {
						toast.success('Food pack successfully updated');
						await invalidateAll();
						await goto('/vendor/menu/?tabValue=food-packs');
					}
				} catch (error) {
					console.error('Failed to update food pack', error);
					toast.error('Failed to update food pack');
				}
			}
		}
	});
...
<div class="container max-w-3xl py-10">
	<div class="mb-6 flex items-center gap-4">
		...
		<div>
			<h1 class="text-2xl font-semibold">Edit Food Pack</h1>
			<p class="text-sm text-muted-foreground">
				Update information for "{data.pack.name}"
			</p>
		</div>
	</div>
	...
	<form use:enhance class="space-y-8">
		<Card.Root>
			<Card.Header>
				...
				<Card.Description>Modify the information for your food pack</Card.Description>
			</Card.Header>
