<script lang="ts">
	...
	import { updatePackSchema } from '@repo/server/validations';

	let {data} = $props();

	const form = superForm(defaults({...data.pack},zod(updatePackSchema)), {
		validators: zod(updatePackSchema),
		SPA: true,
		onUpdate: async ({ form }) => {
			if (form.valid) {
				try {
					const res = await client.vendor.pack[':id'].$patch({
